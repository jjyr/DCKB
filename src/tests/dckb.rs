...
fn test_dckb_withdraw() {
    ...
    let (dckb_cell, dckb_previous_out_point, dckb_cell_data) =
        gen_dckb_cell(&mut data_loader, 123468105678, 0, lock_args);
...
fn test_dckb_transfer() {
    ...
    let (dckb_cell1, dckb_previous_out_point1, dckb_cell_data1) =
        gen_dckb_cell(&mut data_loader, sender_coin, 0, lock_args.clone());
    let (dckb_cell2, dckb_previous_out_point2, dckb_cell_data2) =
        gen_dckb_cell(&mut data_loader, receiver_coin, 1554, lock_args);
...
fn test_dckb_deposit() {
    ...
    let (output_cell, _) = gen_dao_cell(
        ...
        gen_dao_lock_lock_script(change_output_cell.lock().calc_script_hash().unpack()),
    );
